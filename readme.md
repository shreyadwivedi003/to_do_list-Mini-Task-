...
- **Sequential Auto-Increment IDs**: Every note is stamped with a static, progressive unique ID number (`1, 2, 3...`) to maintain permanent reference paths.
...
Restarting the server everytime you make a change can be tedious and irritating , so for that we can use the command:
  ```bash
  npx nodemon server.js


The application will successfully boot up and listen for incoming HTTP requests locally at:
http://localhost:3000
